...
from server.users.services import user_unique_session

class TestUserUniqueSession(TestCase):
    def setUp(self):
        self.service = user_unique_session
    
    @ patch('server.users.services.user_unique_session')
    def test_service_return_none_when_not_user(self, user_unique_session_mock):
